...
const user = {
  state: {
    token: getToken(), id: '', name: '', avatar: '', roles: [], permissions: []
  },
    ...
    GetInfo({commit, state}) {
      return new Promise((resolve, reject) => {
        getInfo().then(res => {
          ...
          if (res.content.roles && res.content.roles.length > 0) { // 验证返回的roles是否是一个非空数组
            commit('SET_ROLES', res.content.roles)
            commit('SET_PERMISSIONS', res.content.permissions)
          } else {
