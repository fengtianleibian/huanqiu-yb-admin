    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['system:notify-template:add']">新增
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="类型" align="center" prop="type">
        <template v-slot="scope">
          <span v-if="scope.row.type === 1"><el-tag type="primary">通知公告</el-tag></span>
          <span v-if="scope.row.type === 2"><el-tag type="info">系统消息</el-tag></span>
          <span v-if="scope.row.type === 3"><el-tag type="warning">订单消息</el-tag></span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="开启状态" align="center" prop="status">
        <template v-slot="scope">
          <span v-if="scope.row.status === 0"><el-tag type="success">启用</el-tag></span>
    ...
    <el-dialog title="发送站内信" :visible.sync="sendNotifyOpen" width="500px" append-to-body>
      <el-form ref="sendNotifyForm" :model="sendNotifyForm" :rules="sendNotifyRules" label-width="140px">
        <el-form-item label="接收人" prop="userId">
          <el-input v-model="sendNotifyForm.userId" placeholder="请输入接收人ID"/>
        </el-form-item>
        <el-form-item label="模板内容" prop="content">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getNotifyTemplatePage(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleSendNotify(row) {
      ...
      this.sendNotifyForm.params = row.parameters;
      this.sendNotifyForm.templateCode = row.code;
      this.sendNotifyForm.templateParams = row.parameters.reduce(function (obj, item) {
        obj[item] = undefined;
        return obj;
      }, {});
      // 根据 row 重置 rules
      this.sendNotifyRules.templateParams = row.parameters.reduce(function (obj, item) {
