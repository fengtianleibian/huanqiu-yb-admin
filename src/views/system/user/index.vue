  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/system/api/sys-user/importData"
      },
      // 查询参数
      queryParams: {
        pageNum: 1,
  ...
  methods: {
    ...
    getList() {
      ...
      listUser(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
          if (response.code === 200) {
            this.userList = response.content.list;
    ...
    getDeptTree() {
      deptTreeSelect().then(response => {
        if (response.code === 200) {
          this.deptOptions = response.content;
        } else {
          this.$modal.msgError(response.message);
        }
      });
    ...
    handleAdd() {
      ...
      getUser().then(response => {
        if (response.code === 200) {
          this.postOptions = response.content.posts;
          this.roleOptions = response.content.roles;
          this.open = true;
          this.title = "添加用户";
    ...
    handleUpdate(row) {
      ...
      getUser(userId).then(response => {
        if (response.code === 200) {
          ...
          this.postOptions = response.content.posts;
          this.roleOptions = response.content.roles;
          this.$set(this.form, "postIds", response.content.postIds);
          this.$set(this.form, "roleIds", response.content.roleIds);
          this.open = true;
          this.title = "修改用户";
          this.form.password = "";
        } else {
